    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white shadow overflow-hidden sm:rounded-lg">
                <form action="{{ route('compliance-covenants.update', $complianceCovenant) }}" method="POST" class="p-6">
                    @csrf
                    @method('PUT')
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    ...
                                    <input type="text" name="issuer_short_name" id="issuer_short_name" 
                                        value="{{ old('issuer_short_name', $complianceCovenant->issuer_short_name) }}" required 
                                ...
                                <div>
                                    ...
                                    <input type="text" name="financial_year_end" id="financial_year_end" 
                                        value="{{ old('financial_year_end', $complianceCovenant->financial_year_end) }}" required
                                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                </div>
                            </div>
                        </div>
                            ...
                            <dl class="grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-4">
                                <div>
                                    ...
                                    <dd class="mt-1 text-sm text-gray-900">
                                        {{ $complianceCovenant->created_at->format('M j, Y H:i') }}
                                    </dd>
                                </div>
                                <div>
                                    <dt class="text-sm font-medium text-gray-500">Last Updated</dt>
                                    <dd class="mt-1 text-sm text-gray-900">
                                        {{ $complianceCovenant->updated_at->format('M j, Y H:i') }}
                                    </dd>
                                </div>
                            </dl>
                        </div>
