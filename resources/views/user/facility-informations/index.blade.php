        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white shadow rounded-lg p-6">
                ...
                <div class="flex flex-col sm:flex-row justify-between gap-4 mb-6">
                    <form method="GET" action="{{ route('facility-informations-info.index') }}" class="w-full sm:w-1/2">
                        <div class="flex gap-2">
                            ...
                            @if($searchTerm)
                                <a href="{{ route('facility-informations-info.index') }}" class="px-4 py-2 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 transition-colors">
                                    Clear
                                </a>
                            @endif
                        </div>
                    ...
                    <a href="{{ route('facility-informations-info.create') }}" class="px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors flex items-center gap-2">
                        ...
                        <tbody class="divide-y divide-gray-200">
                            @forelse($facilities as $facility)
                                <tr class="hover:bg-gray-50">
                                    ...
                                    <td class="px-6 py-4">{{ $facility->instrument_type }}</td>
                                    <td class="px-6 py-4 space-x-2">
                                        <a href="{{ route('facility-informations-info.show', $facility) }}" 
                                        ...
                                        <form action="{{ route('facility-informations-info.destroy', $facility) }}" method="POST" class="inline">
                                            @csrf @method('DELETE')
                                            <button type="submit" 
