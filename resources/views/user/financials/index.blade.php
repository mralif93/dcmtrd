<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <a href="{{ route('financials-info.create') }}" class="inline-flex items-center px-4 py-2 bg-blue-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-blue-700 focus:bg-blue-700 active:bg-blue-800 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150">
                Add New Financial
            </a>
        </div>
    </x-slot>
                    ...
                    <div class="space-y-4">
                        <div class="relative">
                            ...
                            <input 
                                ...
                                name="search" 
                                value="{{ $search ?? '' }}" 
                                placeholder="Search by portfolio, bank, type, purpose..." 
                        ...
                        <div class="flex justify-end items-center pt-2 space-x-3">
                            ...
                            @if(isset($search) || ($status ?? '') != 'all' || ($type ?? '') != 'all')
                                <a 
                    ...
                    <table class="w-full text-sm text-left">
                        ...
                        <tbody class="divide-y divide-gray-200">
                            @forelse ($financials as $financial)
                                <tr>
                                    <td class="px-6 py-4 font-medium text-gray-900">{{ $financial->portfolio->portfolio_name }}</td>
                                    <td class="px-6 py-4">{{ $financial->bank->name }}</td>
                                    <td class="px-6 py-4">{{ $financial->financialType->name }}</td>
                                    <td class="px-6 py-4">{{ $financial->purpose }}</td>
                                    <td class="px-6 py-4">{{ number_format($financial->total_facility_amount, 2) }}</td>
                                    <td class="px-6 py-4">
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex space-x-2">
                                            ...
                                            <a href="{{ route('financials-info.edit', $financial) }}" class="text-yellow-600 hover:text-yellow-900">
                                                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            ...
                                            <form method="POST" action="{{ route('financials-info.destroy', $financial) }}" class="inline">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="text-red-600 hover:text-red-900" onclick="return confirm('Are you sure you want to delete this financial record?')">
                                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            ...
                            @empty
                                <tr>
                                    <td colspan="7" class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 text-center">No financial records found {{ request('search') ? 'matching your search' : '' }}</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
