            ...
            <div class="bg-white shadow rounded-lg p-6">
                ...
                <div class="flex flex-col sm:flex-row justify-between gap-4 mb-6">
                    ...
                    <div class="flex space-x-2">
                        <a href="{{ route('payment-schedules-info.upload-form') }}" class="px-4 py-2 bg-gray-600 text-white rounded-lg hover:bg-gray-700 transition-colors flex items-center gap-2">
                            Upload Data
                        </a>
                        
                        <a href="{{ route('payment-schedules-info.create') }}" class="px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors flex items-center gap-2">
                            ...
                            @forelse($paymentSchedules as $schedule)
                                <tr class="hover:bg-gray-50">
                                    ...
                                    <td class="px-6 py-4 space-x-2">
                                        <a href="{{ route('payment-schedules-info.show', $schedule) }}" 
                                        class="px-3 py-1 bg-blue-100 text-blue-800 rounded-md hover:bg-blue-200 transition-colors">
                                            View
                                        </a>
                                        <a href="{{ route('payment-schedules-info.edit', $schedule) }}" 
                                        class="px-3 py-1 bg-green-100 text-green-800 rounded-md hover:bg-green-200 transition-colors">
                                            Edit
                                        </a>
                                        <form action="{{ route('payment-schedules-info.destroy', $schedule) }}" method="POST" class="inline">
                                            @csrf @method('DELETE')
                                            <button type="submit" 
