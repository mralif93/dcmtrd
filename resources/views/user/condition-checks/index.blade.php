            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <div class="flex justify-between items-center mb-6">
                        ...
                        <a href="{{ route('condition-checks-info.create') }}" class="px-4 py-2 bg-indigo-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-indigo-700">
                            {{ __('Add New') }}
                        </a>
                    </div>
                                    ...
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                            <div class="flex space-x-2">
                                                <a href="{{ route('condition-checks-info.show', $check->id) }}" class="text-indigo-600 hover:text-indigo-900">View</a>
                                                <a href="{{ route('condition-checks-info.edit', $check->id) }}" class="text-yellow-600 hover:text-yellow-900">Edit</a>
                                                <form action="{{ route('condition-checks-info.destroy', $check->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this condition check?');" class="inline">
                                                    @csrf
                                                    @method('DELETE')
