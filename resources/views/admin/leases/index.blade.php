                        ...
                        <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                            ...
                            <div class="flex items-end">
                                ...
                                @if(request('search') || request('tenant_id') || request('status'))
                                    <a href="{{ route('leases.index') }}" class="ml-2 inline-flex items-center px-4 py-2 bg-gray-100 border border-gray-300 rounded-md font-medium text-gray-700 hover:bg-gray-200">
                                        Clear
                                    </a>
                                @endif
                            </div>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @forelse ($leases as $lease)
                                <tr>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm text-gray-500">{{ $lease->tenant->name }}</div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm text-gray-500">
                                            {{ $lease->start_date->format('d/m/Y') }} to {{ $lease->end_date->format('d/m/Y') }}
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm text-gray-500">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span class="px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full {{ $lease->status === 'active' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <div class="flex justify-end space-x-2">
                                            ...
                                            <form action="{{ route('leases.destroy', $lease) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this lease?');" class="inline">
                                                @csrf
                                                @method('DELETE')
