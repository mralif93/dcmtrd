<!-- resources/views/admin/checklist-items/edit.blade.php -->
<x-app-layout>
    <x-slot name="header">
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    <form action="{{ route('checklist-items.update', $item->id) }}" method="POST" class="space-y-6" id="itemForm">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <select id="section" name="section" class="mt-1 block w-full rounded-md border-gray-300" required>
                                    ...
                                    @foreach($checklist->sections as $section)
                                        <option value="{{ $section }}" {{ old('section', $item->section) == $section ? 'selected' : '' }}>
                                            {{ $section }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <select id="type" name="type" class="mt-1 block w-full rounded-md border-gray-300" required>
                                    ...
                                    <option value="Boolean" {{ old('type', $item->type) == 'Boolean' ? 'selected' : '' }}>Yes/No</option>
                                    ...
                                    <option value="Rating" {{ old('type', $item->type) == 'Rating' ? 'selected' : '' }}>Rating</option>
                                </select>
                                @error('type')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            ...
                            <div>
                                ...
                                <input type="number" id="order" name="order" value="{{ old('order', $item->order) }}" 
                            ...
                            <div class="col-span-2">
                                ...
                                <textarea id="description" name="description" rows="3" 
                                    class="mt-1 block w-full rounded-md border-gray-300">{{ old('description', $item->description) }}</textarea>
                                @error('description')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div>
                                <label for="required" class="inline-flex items-center">
                                    <input type="checkbox" id="required" name="required" value="1" 
                                        {{ old('required', $item->required) ? 'checked' : '' }}
                        ...
                        <div id="ratingOptionsSection" class="mt-6 {{ old('type', $item->type) === 'Rating' ? '' : 'hidden' }}">
                            ...
                            <div id="ratingOptions" class="space-y-2">
                                @if(old('options'))
                                    ...
                                    @endforeach
                                @elseif($item->options && $item->type === 'Rating')
