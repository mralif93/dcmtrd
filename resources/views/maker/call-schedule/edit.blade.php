    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white shadow overflow-hidden sm:rounded-lg">
                <form action="{{ route('call-m.update', $call) }}" method="POST" class="p-6">
                    @csrf
                    @method('PUT')
                    <div class="space-y-6 pb-6">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    ...
                                    <select name="redemption_id" id="redemption_id" required
                                        ...
                                        @foreach($redemptions as $redemption)
                                            <option value="{{ $redemption->id }}" 
                                                @selected(old('redemption_id', $call->redemption_id) == $redemption->id)>
                                ...
                                <div>
                                    ...
                                    <input type="number" name="call_price" id="call_price" 
                                        value="{{ old('call_price', $call->call_price) }}" 
                                        required 
                                        step="0.01"
                                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    ...
                                    <input type="date" name="start_date" id="start_date" 
                                        value="{{ old('start_date', $call->start_date->format('Y-m-d')) }}" 
                                        required
                                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500
                                ...
                                <div>
                                    ...
                                    <input type="date" name="end_date" id="end_date" 
                                        value="{{ old('end_date', $call->end_date->format('Y-m-d')) }}" 
                                        required
                                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500
                            ...
                            <dl class="grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-4">
                                <div>
                                    ...
                                    <dd class="mt-1 text-sm text-gray-900">
                                        {{ $call->created_at->format('M j, Y H:i') }}
                                    </dd>
                                </div>
                                <div>
                                    <dt class="text-sm font-medium text-gray-500">Last Updated</dt>
                                    <dd class="mt-1 text-sm text-gray-900">
                                        {{ $call->updated_at->format('M j, Y H:i') }}
                                    </dd>
                                </div>
                            </dl>
                        </div>
                        ...
                        <div class="flex justify-end gap-4 border-t border-gray-200 pt-6">
                            <a href="{{ route('bond-m.show', $call->redemption->bond) }}" 
