    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                {{ __('Disposal Installations List') }}
            </h2>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white shadow rounded-lg p-6 mb-6 border border-gray-100">
                ...
                <form method="GET" action="{{ route('checklist-disposal-installations.index') }}">
                    <div class="space-y-4">
                        <div class="relative">
                            ...
                            <input 
                                ...
                                name="search" 
                                value="{{ $search ?? '' }}" 
                                placeholder="Search by checklist, prepared by..." 
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                            ...
                            <div class="relative">
                                ...
                                >
                                    ...
                                    <option value="not_verified" {{ ($verified ?? '') === 'not_verified' ? 'selected' : '' }}>Not Verified</option>
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="flex justify-end items-center pt-2 space-x-3">
                            ...
                            @if(isset($search) || isset($status) && $status != '' || isset($date) && $date != '' || isset($verified) && $verified != '')
                                <a 
                                    href="{{ route('checklist-disposal-installations.index') }}" 
            ...
            <div class="bg-white shadow rounded-lg p-6">
                ...
                <div class="overflow-auto">
                    <table class="w-full text-sm text-left">
                        ...
                        <tbody class="divide-y divide-gray-200">
                            @forelse ($disposalInstallations as $installation)
                                <tr>
                                    <td class="px-6 py-4 font-medium text-gray-900">
                                        {{ $installation->checklist->id }}
                                    </td>
                                    <td class="px-6 py-4">
                                        <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ $installation->status_badge_class }}">
                                            {{ ucfirst($installation->status) }}
                                        </span>
                                    </td>
                                    <td class="px-6 py-4">
                                        {{ $installation->prepared_by ?? 'Not assigned' }}
                                    </td>
                                    <td class="px-6 py-4">
                                        {{ $installation->verified_by ?? 'Not verified' }}
                                    </td>
                                    <td class="px-6 py-4">
                                        {{ $installation->approval_datetime ? $installation->approval_datetime->format('M d, Y H:i') : 'Not approved' }}
                                    </td>
                                    <td class="px-6 py-4">
                                        <div class="flex space-x-2">
                                            <a href="{{ route('checklist-disposal-installations.show', $installation) }}" class="text-indigo-600 hover:text-indigo-900">
                                                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                                                </svg>
                                            </a>
                                            <a href="{{ route('checklist-disposal-installations.edit', $installation) }}" class="text-yellow-600 hover:text-yellow-900">
                                                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            ...
                            @empty
                                <tr>
                                    <td colspan="6" class="px-6 py-4 text-sm text-gray-500 text-center">No disposal installations found {{ request('search') ? 'matching your search' : '' }}</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
                ...
                <!-- Pagination -->
                @if(isset($disposalInstallations) && method_exists($disposalInstallations, 'links'))
                <div class="mt-4">
                    {{ $disposalInstallations->links() }}
                </div>
                
                <!-- Results count -->
                <div class="mt-2 text-sm text-gray-500">
                    Showing {{ $disposalInstallations->firstItem() ?? 0 }} to {{ $disposalInstallations->lastItem() ?? 0 }} of {{ $disposalInstallations->total() }} disposal installations
                </div>
                @endif
            </div>
        </div>
