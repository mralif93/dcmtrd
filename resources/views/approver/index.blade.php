        ...
        <div class="hidden py-12 dashboard-section" id="dcmtrd-section" data-section="dcmtrd">
            <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
                ...
                <div class="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
                    <!-- Trustee Fees -->
                    <x-dashboard-card title="Trustee Fees" icon="receipt-refund" :count="$trusteeFeesCount ?? 0" :href="route('trustee-fee-a.index')"
                        color="bg-blue-100" />

                    <!-- Compliance Covenants -->
                    <x-dashboard-card title="Compliance Covenants" icon="document-check" :count="$complianceCovenantCount ?? 0"
                        :href="route('compliance-covenant-a.index')" color="bg-blue-100" />
                    ...
                    <div x-data="{ status: '{{ request('status') }}' }" class="bg-white border-b border-gray-200">
                        <nav class="flex px-4 py-3 space-x-2 text-sm font-medium text-gray-500" aria-label="Tabs">
                            ...
                            @foreach ($statuses as $value => $label)
                                <a x-bind:class="status === '{{ $value }}' ? 'bg-indigo-100 text-indigo-700' :
                                    'hover:bg-gray-100 text-gray-600'"
                                    href="{{ route('approver.dashboard', array_merge(request()->except('page'), ['status' => $value])) }}"
                                    ...
                                    : ($issuer->status == 'Pending'
                                        ...
                                        <td class="px-6 py-4 text-sm font-medium text-center whitespace-nowrap">
                                            <div class="flex justify-end space-x-2">
                                                <a href="{{ route('issuer-a.show', $issuer) }}"
    ...
    @if (Auth::user()->hasPermission('REITS'))
        <div class="hidden py-12 dashboard-section" id="reits-section" data-section="reits">
            <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
                ...
                <div class="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
                    <!-- Properties -->
                    <x-dashboard-card title="Properties" icon="office-building" :count="$propertiesCount ?? 0" :pendingCount="$pendingPropertiesCount ?? 0"
                        :href="route('property-a.main', ['tab' => 'all'])" color="bg-green-100" />

                    <!-- Financials -->
                    <x-dashboard-card title="Financials" icon="document-chart-bar" :count="$financialsCount ?? 0"
                        :pendingCount="$pendingFinancialsCount ?? 0" :href="route('financial-a.main', ['tab' => 'all'])" color="bg-green-100" />
                    ...
                    <!-- Lease -->
                    <x-dashboard-card title="Lease" icon="document-duplicate" :count="$leaseCount ?? 0" :pendingCount="$pendingLeaseCount ?? 0"
                        :href="route('lease-a.main', ['tab' => 'all'])" color="bg-green-100" />

                    <!-- Site Visit -->
                    <x-dashboard-card title="Site Visit" icon="location-marker" :count="$siteVisitCount ?? 0" :pendingCount="$pendingSiteVisitCount ?? 0"
                        :href="route('site-visit-a.main', ['tab' => 'all'])" color="bg-green-100" />

                    <!-- Checklist -->
                    <x-dashboard-card title="Checklist" icon="clipboard-check" :count="$checklistCount ?? 0" :pendingCount="$pendingChecklistCount ?? 0"
                    ...
                    <x-dashboard-card title="Appointment" icon="calendar" :count="$appointmentsCount ?? 0" :pendingCount="$pendingAppointmentsCount ?? 0"
                        :href="route('appointment-a.main', ['tab' => 'all'])" color="bg-green-100" />

                    <!-- Approval Form -->
                    <x-dashboard-card title="Approval Form" icon="document-check" :count="$approvalFormsCount ?? 0"
                        :pendingCount="$pendingApprovalFormsCount ?? 0" :href="route('approval-form-a.main', ['tab' => 'all'])" color="bg-green-100" />

                    <!-- Approval Property -->
                    <x-dashboard-card title="Approval Property" icon="check-circle" :count="$approvalPropertiesCount ?? 0"
                        :pendingCount="$pendingApprovalPropertiesCount ?? 0" :href="route('approval-property-a.main', ['tab' => 'all'])" color="bg-green-100" />

                    <!-- Activity Diary -->
                    <x-dashboard-card title="Activity Diary" icon="clipboard-list" :count="$siteVisitLogsCount ?? 0"
                        :pendingCount="$pendingSiteVisitLogsCount ?? 0" :href="route('site-visit-log-a.main', ['tab' => 'all'])" color="bg-green-100" />
                </div>
                                ...
                                <div class="flex items-end">
                                    ...
                                    @if (request('search') || request('status'))
                                        <a href="{{ route('maker.dashboard', ['section' => 'reits']) }}"
                                            class="inline-flex items-center px-4 py-2 ml-2 text-sm font-medium text-gray-700 bg-gray-100 border border-gray-300 rounded-md hover:bg-gray-200">
                                            Clear
                                        </a>
                                    @endif
                                </div>
...
$statusColors = [
                                        ...
                                        $badgeClass = $statusColors[$normalizedStatus] ?? 'bg-gray-100 text-gray-800'; @endphp
                                        {{ $badgeClass }}">
