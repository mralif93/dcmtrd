<x-app-layout>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white shadow overflow-hidden sm:rounded-lg">
                <div class="px-4 py-5 sm:px-6 flex justify-between items-center">
                    <h3 class="text-lg font-medium text-gray-900">Financials</h3>
                </div>

                <!-- Status Tabs -->
                <div class="border-b border-gray-200">
                    <nav class="-mb-px flex px-6 space-x-6">
                        <a href="{{ route('financial-a.main', ['tab' => 'all'] + request()->except('page', 'tab')) }}" 
                        ...
                        <a href="{{ route('financial-a.main', ['tab' => 'pending'] + request()->except('page', 'tab')) }}" 
                        ...
                        <a href="{{ route('financial-a.main', ['tab' => 'rejected'] + request()->except('page', 'tab')) }}" 
                        ...
                        <a href="{{ route('financial-a.main', ['tab' => 'active'] + request()->except('page', 'tab')) }}" 
                ...
                <div class="px-4 py-3 bg-gray-50 sm:px-6">
                    <form action="{{ route('financial-a.main') }}" method="GET" class="grid grid-cols-1 gap-4 md:grid-cols-3">
                        ...
                        <div>
                            <label for="financial_type" class="block text-sm font-medium text-gray-700">Financial Type</label>
                            <select id="financial_type" name="financial_type" class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                                <option value="">All Types</option>
                                @foreach($financialTypes as $type)
                                    <option value="{{ $type->id }}" {{ request('financial_type') == $type->id ? 'selected' : '' }}>{{ $type->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="flex items-end space-x-3">
                            ...
                            <a href="{{ route('financial-a.main', ['tab' => $activeTab]) }}" class="inline-flex items-center px-4 py-2 border border-gray-300 shadow-sm text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                                Reset
                            </a>
                        </div>
                    </form>
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Portfolio & Bank</th>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            ...
                            <tr>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm text-gray-900">{{ Str::limit($financial->purpose, 30) }}</div>
                                    <div class="text-sm text-gray-500">{{ $financial->batch_no ?? 'N/A' }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    ...
                                    <div class="text-sm text-gray-500">
                                        Utilized: RM {{ number_format($financial->utilization_amount, 2) }}
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    ...
                                    <div class="flex justify-end space-x-2">
                                        ...
                                        @if($financial->status == 'pending')
                                            ...
                                            <form method="POST" action="{{ route('financial-a.approve', $financial) }}" class="inline">
                                                @csrf
                                                <button type="submit" class="text-green-600 hover:text-green-900" title="Approve Financial">
                                            ...
                                            <button onclick="openRejectModal('{{ $financial->id }}')" class="text-red-600 hover:text-red-900" title="Reject Financial">
                                                <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            ...
                            <tr>
                                <td colspan="5" class="px-6 py-4 text-center">
                                    <div class="text-sm text-gray-500">No financial records found.</div>
                                </td>
                            </tr>
                            @endif
                        </tbody>
            ...
            <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                <div class="sm:flex sm:items-start">
                    ...
                    <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                        <h3 class="text-lg leading-6 font-medium text-gray-900">Reject Financial</h3>
                        <div class="mt-2">
                            <p class="text-sm text-gray-500">Please provide a reason for rejecting this financial record.</p>
                        </div>
                    </div>
                </div>
    ...
    <script>
        ...
        function openRejectModal(financialId) {
            document.getElementById('rejectForm').action = `/approver/financial/${financialId}/reject`;
